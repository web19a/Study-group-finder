import { getCurrentUser, isCurrentUserTeacher } from './auth.js';
import { getGroups, getUsers } from './data.js';
import { formatMeetingTime } from './utils.js';
 ...
 */
export function renderAuthForm() { // Exported for app.js to use
...
function renderGroupCard(group, currentUser) {
    ...
    // Teachers get specific moderation options if they are the creator or explicitly joined
    // For now, we'll only show moderation for creator. We'll refine teacher role later.
    // The "Teacher Dashboard" tab will handle broader teacher functionalities.
    // If a teacher wants to moderate a specific group, they'd ideally be a member of it.
    let teacherModerationHtml = '';
    if (isTeacher && (isCreator || isMember)) { // Teachers can moderate groups they created or joined
        teacherModerationHtml = `
            <div class="teacher-options">
                <button class="btn btn-secondary btn-small" data-action="manageRequests" data-group-id="${group.id}">Manage Requests (${group.joinRequests.length})</button>
                <button class="btn btn-secondary btn-small" data-action="addResource" data-group-id="${group.id}">Add Resource</button>
                <button class="btn btn-secondary btn-small" data-action="addAnnouncement" data-group-id="${group.id}">Add Announcement</button>
            </div>
        `;
    }
...
export function loadAllGroups() {
    ...
    if (!currentUser) { // Should not happen if dashboard is rendered
...
/**
 * Renders the teacher-specific dashboard, showing created groups and pending requests.
 */
export function loadTeacherDashboard() {
    ...
    const teacherCreatedGroups = allGroups.filter(group => group.creatorId === currentUser.id);

    let teacherDashboardHtml = `
        <h3>Your Created Study Groups</h3>
        <div class="teacher-groups-list">
    `;

    if (teacherCreatedGroups.length === 0) {
        teacherDashboardHtml += '<p class="info-text">You haven\'t created any groups yet. Go to "Create New Group" to get started!</p>';
    } else {
        let teacherCreatedHtml = teacherCreatedGroups.map(group => {
            let requestsHtml = '';
            if (group.joinRequests.length > 0) {
                ...
                requestsHtml = '<p class="info-text-small">No pending join requests for this group.</p>';
            }

            return `
                <div class="teacher-group-card">
                    <div class="teacher-group-card-header">
    ...
    teacherDashboardHtml += `</div>`; // Close teacher-groups-list

    dashboardContent.innerHTML = teacherDashboardHtml;
}
